...
public final class Filmstrip {
	...
	public Filmstrip(Minecraft minecraft, Path ffmpeg, Path file, double duration, int count) {
		this.minecraft = minecraft;
		this.count = Math.max(1, count);
		this.duration = Math.max(0.01, duration);
		this.ids = new Identifier[this.count];
		this.textures = new DynamicTexture[this.count];
		this.tw = new int[this.count];
		this.th = new int[this.count];
		for (int i = 0; i < this.count; i++) {
			ids[i] = Identifier.fromNamespaceAndPath("clipify", "strip_" + Long.toHexString(System.nanoTime()) + "_" + i);
		}
	...
	private void extract(Path ffmpeg, Path file, double duration) {
		for (int i = 0; i < count && !closed; i++) {
			...
			minecraft.execute(() -> {
				...
				minecraft.getTextureManager().register(ids[slot], textures[slot]);
	...
	public void draw(GuiGraphicsExtractor graphics, int x, int y, int w, int h) {
		for (int i = 0; i < count; i++) {
			...
			if (textures[i] != null) {
				graphics.blit(RenderPipelines.GUI_TEXTURED, ids[i], cellX, y, 0f, 0f, cellW, h, tw[i], th[i], tw[i], th[i]);
			} else {
				graphics.fill(cellX, y, cellX + cellW, y + h, (i & 1) == 0 ? 0xFF141414 : 0xFF1B1B1B);
			}
			graphics.fill(cellEnd - 1, y, cellEnd, y + h, 0xFF000000);
		}
	}
	 ...
	 */
	public boolean drawFrameAt(GuiGraphicsExtractor graphics, double t, int x, int y, int w, int h) {
		...
		int dx = x + (w - dw) / 2;
		int dy = y + (h - dh) / 2;
		graphics.blit(RenderPipelines.GUI_TEXTURED, ids[found], dx, dy, 0f, 0f, dw, dh, fw, fh, fw, fh);
		return true;
	}
	...
	public void close() {
		...
		for (int i = 0; i < count; i++) {
			if (textures[i] != null) {
				minecraft.getTextureManager().release(ids[i]);
				textures[i].close();
				textures[i] = null;
			}
		}
