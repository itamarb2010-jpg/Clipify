 ...
 * to a single reusable {@link DynamicTexture} (created once from the first frame, then
 * {@code setPixels}+{@code upload} per frame — no per-frame texture churn).
 *
...
public final class VideoPlayer {
	...
	public VideoPlayer(Minecraft minecraft, Path ffmpeg, Path file, double duration) {
		this.minecraft = minecraft;
		this.ffmpeg = ffmpeg;
		this.file = file;
		this.duration = Math.max(0.01, duration);
		this.textureId = Identifier.fromNamespaceAndPath("clipify", "video_" + Long.toHexString(System.nanoTime()));
	...
	private void display(NativeImage img, double time) {
		...
		if (texture == null) {
			...
			texture.setPixels(img); // closes the previous image
			texture.upload();
		} else {
			minecraft.getTextureManager().release(textureId);
			texture.close();
