 ...
 * copies the image to the system clipboard (no upload, no link) so it can be pasted anywhere. The
 * framebuffer grab runs on the render thread; the clipboard copy is bounced onto a worker thread.
 *
...
public final class Screenshotter {
	...
	public static void capture(MinecraftClient client, ClipifyConfig config) {
		if (client == null) {
			return;
		}
		client.execute(() -> {
			try {
				ScreenshotRecorder.takeScreenshot(client.getFramebuffer(), image -> onImage(client, config, image));
			} catch (Throwable t) {
	...
	private static void onImage(MinecraftClient client, ClipifyConfig config, NativeImage image) {
		Path file;
		try {
		...
		Thread worker = new Thread(() -> {
			...
			Notifications.info(client,
					Text.translatable(copied ? "clipify.toast.screenshot" : "clipify.toast.screenshot.saved"),
