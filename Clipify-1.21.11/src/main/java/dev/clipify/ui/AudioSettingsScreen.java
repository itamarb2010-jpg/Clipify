...
public final class AudioSettingsScreen extends Screen {
	...
	private CyclingButtonWidget<String> micDeviceCycle; // scroll-managed vanilla widget
	private ButtonWidget startTestButton;               // scroll-managed vanilla widget
	...
	protected void init() {
		...
		for (int i = 0; i < labels.length; i++) {
			...
			int idx = i;
			ButtonWidget b = ButtonWidget.builder(Text.literal(labels[i]), btn -> {
				if (idx != 2) {
					parent.openTab(idx);
				}
			}).dimensions(x, TAB_BAR_Y, tabW, 20).build();
		...
		addDrawableChild(ButtonWidget.builder(Text.translatable("clipify.config.save"), b -> parent.applyAndClose())
				.dimensions(left, this.height - 32, WIDGET_WIDTH, 20).build());
		...
		this.micDeviceCycle = CyclingButtonWidget.<String>builder(Text::literal, cfg.micDevice)
				.values(micOptions)
				.build(left, 0, WIDGET_WIDTH, 20, Text.translatable("clipify.audio.microphone"),
						(btn, v) -> cfg.micDevice = v);
		this.micDeviceCycle.visible = false;
		addSelectableChild(this.micDeviceCycle);

		this.startTestButton = ButtonWidget.builder(Text.translatable("clipify.audio.start_test"),
				b -> toggleMicTest()).dimensions(left, 0, WIDGET_WIDTH, 20).build();
		this.startTestButton.visible = false;
		addSelectableChild(this.startTestButton);
	}
	...
	public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
		super.render(ctx, mouseX, mouseY, delta); // vanilla background + tab buttons + Save button
		...
		ctx.fill(tx, TAB_BAR_Y + 20, tx + tabW, TAB_BAR_Y + 22, ACCENT_COLOR);
		...
		ctx.enableScissor(0, viewTop, this.width, viewBottom);
		int y = viewTop - scrollY;
		drawRecording(ctx, left, y, mouseX, mouseY);
		if (micOn) {
			drawMic(ctx, left, y + recH + GAP_SECTION, mouseX, mouseY);
		}
		ctx.disableScissor();
		drawScrollbar(ctx);
	}
	...
	private void drawRecording(DrawContext ctx, int x, int y, int mouseX, int mouseY) {
		int iw = WIDGET_WIDTH;
		int cy = y + PADY;
		header(ctx, x, cy, iw, "clipify.audio.recording");
		cy += HDR_H;

		sourceRow(ctx, x, iw, cy, "mic", "clipify.audio.microphone", cfg.captureMicrophone,
		...
		if (isMac()) {
			...
			ctx.drawTextWithShadow(this.textRenderer, Text.translatable("clipify.audio.pc_mac_warning"),
					x, cy + 6, MAC_WARN);
			return;
		}

		sourceRow(ctx, x, iw, cy, "headphones", "clipify.audio.pc_audio", cfg.captureGameAudio,
		...
		for (String dev : outputDevices) {
			...
			checkbox(ctx, devX, cy + 2, 11, on, hov);
			ctx.drawTextWithShadow(this.textRenderer, Text.literal(trimTo(dev, iw - 44)), devX + 17, cy + 4,
					on ? TEXT : MUTED);
			final String d = dev;
	...
	private void sourceRow(DrawContext ctx, int x, int iw, int cy, String icon, String labelKey, boolean enabled,
	                       Runnable toggle, int vol, SliderId id, IntConsumer setVol, int mouseX, int mouseY) {
		int mid = cy + ROW / 2;
		boolean hov = mouseX >= x && mouseX <= x + iw && mouseY >= cy && mouseY <= cy + ROW;
		checkbox(ctx, x, mid - 6, 13, enabled, hov);
		clickables.add(new Clickable(x, cy, 20, ROW, toggle));
		icon(ctx, icon, x + 20, mid - 6, 13, enabled ? TEXT : MUTED);
		ctx.drawTextWithShadow(this.textRenderer, Text.translatable(labelKey), x + 40, mid - 4, enabled ? TEXT : MUTED);

		int valW = 26;
		int sliderW = 110;
		int sx = x + iw - valW - sliderW;
		slider(ctx, sx, mid, sliderW, vol, enabled, id, setVol, mouseX, mouseY);
		String pct = vol + "%";
		ctx.drawTextWithShadow(this.textRenderer, Text.literal(pct), x + iw - this.textRenderer.getWidth(pct),
				mid - 4, enabled ? TEXT : MUTED);
	}

	private void drawMic(DrawContext ctx, int x, int y, int mouseX, int mouseY) {
		int iw = WIDGET_WIDTH;
		int cy = y + PADY;
		header(ctx, x, cy, iw, "clipify.audio.mic_settings");
		cy += HDR_H;

		micDeviceCycle.setY(cy);
		micDeviceCycle.visible = cy >= viewTop && cy + 20 <= viewBottom;
		if (micDeviceCycle.visible) {
			micDeviceCycle.render(ctx, mouseX, mouseY, renderDelta);
		}
		cy += 24 + 6;

		startTestButton.setMessage(Text.translatable(micTesting ? "clipify.audio.stop_test" : "clipify.audio.start_test"));
		startTestButton.setY(cy);
		startTestButton.visible = cy >= viewTop && cy + 20 <= viewBottom;
		if (startTestButton.visible) {
			startTestButton.render(ctx, mouseX, mouseY, renderDelta);
		}
		cy += 24 + 6;

		waveform(ctx, x, cy, iw, 28);
	}

	// --------------------------------------------------------------- primitives

	private void header(DrawContext ctx, int x, int cy, int iw, String key) {
		ctx.drawTextWithShadow(this.textRenderer, Text.translatable(key), x, cy, TEXT);
		ctx.fill(x, cy + 13, x + iw, cy + 14, DIVIDER);
	}

	/** Neutral checkbox: bordered box, white check when ticked (no colour accent). */
	private void checkbox(DrawContext ctx, int x, int y, int size, boolean checked, boolean hover) {
		roundRect(ctx, x, y, size, size, 2, hover ? BOX_HOV : BOX_BG);
		roundRectOutline(ctx, x, y, size, size, 2, hover ? 0xFF6A6F79 : BORDER);
		if (checked) {
			icon(ctx, "check", x + 1, y + 1, size - 2, TEXT);
		}
	}

	private void slider(DrawContext ctx, int x, int cy, int w, int value, boolean enabled, SliderId id,
	                    IntConsumer set, int mouseX, int mouseY) {
		int trackY = cy - 1;
		roundRect(ctx, x, trackY, w, 3, 1, TRACK);
		int knobX = x + Math.round(value / 200f * w); // volume slider is 0–200 % (100 = unity)
		if (knobX > x) {
			roundRect(ctx, x, trackY, knobX - x, 3, 1, enabled ? SLIDER_ON : SLIDER_OFF);
		}
		boolean hot = mouseX >= x - 4 && mouseX <= x + w + 4 && mouseY >= cy - 8 && mouseY <= cy + 8;
		int r = hot || dragging == id ? 6 : 5;
		roundRect(ctx, knobX - r, cy - r, 2 * r, 2 * r, r, enabled ? 0xFFFFFFFF : 0xFFB6BAC2);
		sliders.add(new SliderRegion(x, cy, w, id, set));
	}

	private void waveform(DrawContext ctx, int x, int y, int w, int h) {
		roundRect(ctx, x, y, w, h, 4, 0xFF101216);
		int step = 4;
		int mid = y + h / 2;
		int bars = (w - 6) / step;
		...
		for (int i = 0, bx = x + 3; i < bars; i++, bx += step) {
			...
			ctx.fill(bx, mid - hh, bx + 2, mid + hh, color);
		}
	}
	...
	private void drawScrollbar(DrawContext ctx) {
		...
		roundRect(ctx, barX, viewTop, 3, viewH, 1, 0xFF1E2127);
		roundRect(ctx, barX, thumbY, 3, thumbH, 1, 0xFF3C414A);
	}
	...
	private void icon(DrawContext ctx, String name, int x, int y, int size, int color) {
		Identifier id = Identifier.of("clipify", "textures/gui/icons/" + name + ".png");
		ctx.drawTexture(RenderPipelines.GUI_TEXTURED, id, x, y, 0f, 0f, size, size, ICON_TEX, ICON_TEX, ICON_TEX, ICON_TEX, color);
	}

	private void roundRect(DrawContext ctx, int x, int y, int w, int h, int r, int color) {
	...
	private void roundRectOutline(DrawContext ctx, int x, int y, int w, int h, int r, int color) {
		r = Math.max(0, Math.min(r, Math.min(w, h) / 2));
	...
	public boolean mouseClicked(Click click, boolean doubled) {
		...
		return super.mouseClicked(click, doubled); // tabs, Save, mic picker, Start Test
	}

	@Override
	public boolean mouseDragged(Click click, double offsetX, double offsetY) {
		if (click.button() == 0 && dragging != SliderId.NONE) {
			applyDrag(click.x());
			return true;
		}
