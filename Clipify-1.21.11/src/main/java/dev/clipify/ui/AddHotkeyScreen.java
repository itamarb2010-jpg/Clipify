...
public final class AddHotkeyScreen extends Screen {
	...
	public AddHotkeyScreen(ClipifyConfigScreen parent, ClipifyConfig working) {
		super(Text.translatable("clipify.config.add_hotkey"));
		this.parent = parent;
	...
	protected void init() {
		...
		addDrawableChild(ButtonWidget.builder(Text.translatable("gui.back"), b -> close())
				.dimensions(x, y + 128, W, 20).build());
	}

	private void addAction(int x, int y, HotkeyAction action, String labelKey) {
		addDrawableChild(ButtonWidget.builder(Text.translatable(labelKey), b -> {
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		context.drawCenteredTextWithShadow(this.textRenderer, this.title,
				this.width / 2, this.height / 2 - 90, 0xFFFFFFFF);
	}
